"""Mzansi Rush — Module 1: Data Ingestion.
...
(JSONL) to data_engineering/data/events/events.jsonl every time
something notable happens in the game.

Why JSONL and not straight into a database:
  - It's dependency-free (stdlib only), so it can't break the game.
  - It's an append-only raw event log — exactly the "extract" source
    Module 2 (orchestration/etl.py) reads from later. Adding new event
    types never requires touching the warehouse or ETL code.

Usage:
    from data_engineering.ingestion.event_logger import EventLogger
...
EVENTS_DIR = os.path.join("data_engineering", "data", "events")

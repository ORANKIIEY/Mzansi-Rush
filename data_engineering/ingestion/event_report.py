"""Quick sanity-check tool for the event log (Module 1: Data Ingestion).

Not part of the game itself — run it from the command line after
playing a bit to see what's been captured:

    python3 -m data_engineering.ingestion.event_report

This is a tiny preview of Module 4 (analytics/). Later, the real ETL
job (orchestration/etl.py) reads events.jsonl, cleans/aggregates it,
and loads it into the warehouse instead of just printing a summary.
"""
...
from data_engineering.ingestion.event_logger import EVENTS_FILE

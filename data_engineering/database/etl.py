"""Mzansi Rush — ETL: load data/events/events.jsonl into the database.

This is Section 9.4 of the report (Extract / Transform / Load) applied
to the actual event log the game now produces.

  Extract   -> read new lines from data/events/events.jsonl
  Transform -> route each event to the right table/columns, upsert
               the Player row, skip malformed lines
  Load      -> write into data/mzansi_rush.db via database/schema.sql

Incremental by design: it remembers the byte offset it last read (in
data/events/.etl_offset) so re-running it after more play sessions only
loads the *new* events — this is the "batch processing" job described
in Section 9.6, meant to be run periodically (or once per session).

Usage:
    python3 -m database.etl            # load new events since last run
    python3 -m database.etl --reset    # reprocess the entire log from scratch
"""
...
from data_engineering.database.db import DB_PATH, get_connection, init_db
